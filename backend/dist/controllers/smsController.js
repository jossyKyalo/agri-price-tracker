...
export const sendSms = async (req, res, next) => {
    try {
        const { recipients, message, sms_type, template_id, template_variables } = req.body;
...
export const getSmsLogs = async (req, res, next) => {
    try {
        ...
        const result = await query(`SELECT sl.*, u.full_name as sent_by_name
       FROM sms_logs sl
       LEFT JOIN users u ON sl.sent_by = u.id
       ${whereClause}
       ORDER BY sl.created_at DESC
       LIMIT $${paramIndex++} OFFSET $${paramIndex++}`, params);
        const countResult = await query(`SELECT COUNT(*) FROM sms_logs ${whereClause}`, params.slice(0, -2));
...
export const createSmsTemplate = async (req, res, next) => {
    try {
        ...
        const result = await query(`INSERT INTO sms_templates (name, template, variables, sms_type, created_by)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`, [name, template, JSON.stringify(variables || []), sms_type, createdBy]);
...
export const getSmsTemplates = async (req, res, next) => {
    try {
        ...
        const result = await query(`SELECT st.*, u.full_name as created_by_name
...
export const updateSmsTemplate = async (req, res, next) => {
    try {
        ...
        const result = await query(`UPDATE sms_templates 
       ...
       WHERE id = $6
       RETURNING *`, [name, template, variables ? JSON.stringify(variables) : null, sms_type, is_active, id]);
...
export const subscribeSms = async (req, res, next) => {
    try {
        const { phone, crops, regions, alert_types } = req.body;
        const userId = req.user?.id;
        const result = await query(`INSERT INTO sms_subscriptions (phone, user_id, crops, regions, alert_types)
       VALUES ($1, $2, $3, $4, $5)
       ON CONFLICT (phone) 
       DO UPDATE SET 
         ...
         is_active = true,
         updated_at = CURRENT_TIMESTAMP
       RETURNING *`, [phone, userId, JSON.stringify(crops || []), JSON.stringify(regions || []), JSON.stringify(alert_types || [])]);
...
export const getSmsSubscriptions = async (req, res, next) => {
    try {
        const { page = 1, limit = 20, is_active = true } = req.query;
        const offset = (Number(page) - 1) * Number(limit);
        const result = await query(`SELECT ss.*, u.full_name as user_name
       FROM sms_subscriptions ss
       LEFT JOIN users u ON ss.user_id = u.id
       WHERE ss.is_active = $1
       ORDER BY ss.created_at DESC
       LIMIT $2 OFFSET $3`, [is_active, limit, offset]);
...
export const unsubscribeSms = async (req, res, next) => {
    try {
        const { phone } = req.params;
        const result = await query('UPDATE sms_subscriptions SET is_active = false, updated_at = CURRENT_TIMESTAMP WHERE phone = $1 RETURNING *', [phone]);
        if (result.rows.length === 0) {
