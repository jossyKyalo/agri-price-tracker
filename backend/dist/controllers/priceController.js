...
export const getPrices = async (req, res, next) => {
    try {
        const { page = 1, limit = 20, crop_id, region_id, market_id, source, verified, date_from, date_to, sort = 'entry_date', order = 'desc' } = req.query;
        ...
        const result = await query(`SELECT pe.*, 
              c.name as crop_name,
       ...
       LIMIT $${paramIndex++} OFFSET $${paramIndex++}`, params);
        // Get total count
        const countResult = await query(`SELECT COUNT(*) FROM price_entries pe ${whereClause}`, params.slice(0, -2) // Remove limit and offset
        );
...
export const createPriceEntry = async (req, res, next) => {
    try {
        const { crop_id, region_id, market_id, price, unit = 'kg', source = 'farmer', notes, entry_date } = req.body;
        const enteredBy = req.user?.id;
        const result = await query(`INSERT INTO price_entries (crop_id, region_id, market_id, price, unit, source, entered_by, notes, entry_date)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       RETURNING id, crop_id, region_id, market_id, price, unit, source, notes, entry_date, created_at`, [crop_id, region_id, market_id, price, unit, source, enteredBy, notes, entry_date || new Date()]);
...
export const updatePriceEntry = async (req, res, next) => {
    try {
        const { id } = req.params;
        const { price, notes, is_verified } = req.body;
        const userId = req.user.id;
        const result = await query(`UPDATE price_entries 
       ...
       WHERE id = $5
       RETURNING *`, [price, notes, is_verified, userId, id]);
...
export const deletePriceEntry = async (req, res, next) => {
    try {
        const { id } = req.params;
...
export const verifyPriceEntry = async (req, res, next) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
        const result = await query(`UPDATE price_entries 
...
export const rejectPriceEntry = async (req, res, next) => {
    try {
        const { id } = req.params;
