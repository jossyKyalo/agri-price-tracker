...
export const sendMessage = async (req, res, next) => {
    try {
        ...
        let conversation;
        if (session_id) {
            const result = await query('SELECT * FROM chat_conversations WHERE session_id = $1', [session_id]);
            if (result.rows.length > 0) {
                conversation = result.rows[0];
            }
            else {
                // Create new conversation
                const newConversation = await query(`INSERT INTO chat_conversations (user_id, session_id, messages, context)
           VALUES ($1, $2, $3, $4)
           RETURNING *`, [userId, session_id, JSON.stringify([]), JSON.stringify(context || {})]);
                conversation = newConversation.rows[0];
            }
        }
        else {
            // Create new conversation without session_id
            const newConversation = await query(`INSERT INTO chat_conversations (user_id, messages, context)
         VALUES ($1, $2, $3)
         RETURNING *`, [userId, JSON.stringify([]), JSON.stringify(context || {})]);
            conversation = newConversation.rows[0];
        }
       ...
       WHERE id = $3`, [JSON.stringify(updatedMessages), JSON.stringify(context || {}), conversation.id]);
...
export const getConversation = async (req, res, next) => {
    try {
        ...
        const result = await query(`SELECT * FROM chat_conversations ${whereClause} ORDER BY updated_at DESC LIMIT 1`, params);
        if (result.rows.length === 0) {
...
export const getUserConversations = async (req, res, next) => {
    try {
        ...
        const result = await query(`SELECT id, session_id, context, created_at, updated_at,
...
export const deleteConversation = async (req, res, next) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
        const result = await query('DELETE FROM chat_conversations WHERE id = $1 AND user_id = $2 RETURNING id', [id, userId]);
        if (result.rows.length === 0) {
