...
export const generatePricePrediction = async (cropId, regionId, predictionDays = 7) => {
    try {
        // Get historical price data
        const historicalData = await query(`SELECT price, entry_date
       FROM price_entries
       WHERE crop_id = $1 AND region_id = $2 AND is_verified = true
       ORDER BY entry_date DESC
       LIMIT 90`, [cropId, regionId]);
        ...
        // Call ML model API
        const response = await axios.post(`${process.env.ML_MODEL_URL}/predict`, requestData, {
            headers: {
...
const generateSimplePrediction = async (cropId, regionId, predictionDays) => {
    try {
        const result = await query(`SELECT price, entry_date
       FROM price_entries
       WHERE crop_id = $1 AND region_id = $2 AND is_verified = true
       ORDER BY entry_date DESC
       LIMIT 30`, [cropId, regionId]);
...
const storePrediction = async (prediction) => {
    try {
        ...
        await query(`INSERT INTO price_predictions (
...
export const getPredictions = async (cropId, regionId, limit = 20) => {
    try {
        const conditions = ['pp.prediction_date >= CURRENT_DATE'];
        const params = [];
        let paramIndex = 1;
        if (cropId) {
        ...
        params.push(limit);
        const result = await query(`SELECT pp.*, c.name as crop_name, r.name as region_name
