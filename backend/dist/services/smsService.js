 ...
 * Send a single SMS
 */
export const sendSmsMessage = async (recipient, message, smsType, sentBy) => {
    try {
        ...
        if (smsClient) {
            ...
            const smsData = response.Recipients ?? response; // SDK shape can vary
        ...
        // Log SMS in database
        const result = await query(`INSERT INTO sms_logs (recipient, message, sms_type, status, external_id, sent_by, sent_at)
       VALUES ($1, $2, $3, $4, $5, $6, CURRENT_TIMESTAMP)
       RETURNING *`, [recipient, message, smsType, status, messageId, sentBy]);
        ...
        // Log failed SMS
        const result = await query(`INSERT INTO sms_logs (recipient, message, sms_type, status, error_message, sent_by)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`, [recipient, message, smsType, 'failed', error.message, sentBy]);
        return result.rows[0];
    }
};
/**
 ...
 */
export const sendBulkSms = async (recipients, message, smsType, sentBy) => {
    const results = [];
 ...
 */
export const getSubscribedNumbers = async (cropIds, regionIds, alertTypes) => {
    try {
        ...
        const result = await query(`SELECT DISTINCT phone FROM sms_subscriptions WHERE ${whereConditions.join(' AND ')}`, params);
 ...
 */
export const sendPriceAlert = async (cropName, price, region, trend, percentage) => {
    try {
        // Load SMS template
        const templateResult = await query('SELECT template FROM sms_templates WHERE name = $1 AND is_active = true', ['Price Alert']);
 ...
 */
export const sendDailyPriceUpdate = async () => {
    try {
 ...
 */
export const processSmsWebhook = async (req) => {
    try {
        const { id, status, phoneNumber } = req.body;
        await query('UPDATE sms_logs SET status = $1, delivered_at = CURRENT_TIMESTAMP WHERE external_id = $2', [status, id]);
