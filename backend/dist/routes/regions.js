...
router.get('/', async (req, res, next) => {
    try {
        ...
        const result = await query(`SELECT * FROM regions ${whereClause} ORDER BY name ASC`, params);
        const response = {
            success: true,
...
router.post('/', authenticate, requireAdmin, validate(schemas.createRegion), async (req, res, next) => {
    try {
        ...
        const result = await query('INSERT INTO regions (name, code, description) VALUES ($1, $2, $3) RETURNING *', [name, code, description]);
        const response = {
            success: true,
...
router.put('/:id', authenticate, requireAdmin, validate(schemas.updateRegion), async (req, res, next) => {
    try {
        ...
        const result = await query(`UPDATE regions 
       ...
       WHERE id = $5
       RETURNING *`, [name, code, description, is_active, id]);
        if (result.rows.length === 0) {
            throw new ApiError('Region not found', 404);
        }
        const response = {
