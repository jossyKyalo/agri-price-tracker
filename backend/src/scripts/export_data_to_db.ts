...
const importKamisData = async () => {
  try {
    ...
    if (!fs.existsSync(DATA_FILE)) {
      ...
      console.log('\nPlease ensure the Python scraper has run.');
      await pool.end();
      process.exit(1);
    }
    ...
    if (!fileContent.trim()) {
      console.log('⚠️  File is empty. No new data to import.');
      console.log('   (The scraper likely found no data newer than the cutoff date)');
      await pool.end();
      process.exit(0);
    }
    ...
    console.log(`📊 Loaded ${records.length} NEW records for import\n`);
    ...
    for (const record of records) {
      if (record.Commodity) {
        uniqueCrops.set(record.Commodity, {
          ...
          cropName: record.CropName || record.Commodity 
        });
      }
      if (record.County) {
    ...
    for (const county of uniqueCounties) {
      ...
      if (mapping) {
        ...
        if (region_id) countyToRegionId.set(county, region_id);
      } else {
        ...
        if (record.Wholesale) {
          ...
          if (wholesalePrice && wholesalePrice > 0) {
            await pool.query(
              ...
              ON CONFLICT DO NOTHING`,  
              [crop_id, marketInfo.region_id, marketInfo.id, wholesalePrice, entryDate]
            );
            wholesaleCount++;
          }
        }
        ...
        if (record.Retail) {
          ...
          if (retailPrice && retailPrice > 0) {
            await pool.query(
              ...
              ON CONFLICT DO NOTHING`, 
              [crop_id, marketInfo.region_id, marketInfo.id, retailPrice, entryDate]
            );
            retailCount++;
          }
        }

        if ((wholesaleCount + retailCount) % 100 === 0) process.stdout.write('.');
      } catch (error) {
        skipped++;
      }
    }
    ...
    console.log('\n✅ KAMIS incremental data sync successful!');
